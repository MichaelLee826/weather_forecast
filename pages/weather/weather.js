  ...
  getWeather: function(cityName) {
    ...
    wx.request({
      ...
      success(data) {
        ...
        todayInfo.temperatureRange = that.getTemperatureRange(lowTemp, highTemp);
  ...
  getForecast: function(cityName) {
    ...
    wx.request({
      ...
      success(data) {
        ...
        //获得今天和明天的小时预报
        var len1 = data.data.data[0].hours.length;
        var len2 = data.data.data[1].hours.length;
        var len = len1 + len2;
        //创建一个数组，用于存放今天和明天的小时预报
        var hoursForecast = new Array(len);
        //今天的小时预报
        for (var i = 0; i < len1; i++) {
          hoursForecast[i] = data.data.data[0].hours[i];
        }
        //明天的小时预报
        for (var i = 0; i < len2; i++) {
          hoursForecast[len1 + i] = data.data.data[1].hours[i];
        }
        //修改天气图标
        for (var i = 0; i < len; i++) {
          hoursForecast[i].wea = that.getIconURL(hoursForecast[i].wea);
        }
